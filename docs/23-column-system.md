...
- **헤더 클릭 = 정렬**: 셀의 **정렬키**로(표시 텍스트와 별개 — 크기/날짜 올바른 정렬).
  - **3상태 순환(★ 사용자 확정 2026-07-04)**: 클릭마다 **오름차순 → 내림차순 → 없음(원래 열거 순서)** 순환. **화살표는 컬럼명 앞**에 표시(▲ 오름 / ▼ 내림 / 무표시 없음). **정렬 순번은 컬럼명 뒤에 원문자**(①②③…). (기존 2상태 토글에서 **3상태로 갱신** — 요청 COL-2.)
...
- 코어 비교자는 이미 키 목록 지원(COL-2a). UI(`OnHeaderTapped`): **Shift+클릭 & 이미 정렬된 컬럼 ≥1**이면 다중열 — `ApplyMultiSort`(없는 키=순번+1 오름 추가 / 정렬 키=방향 순환, 없음이면 제거·뒤 순번 당김). 그 외(Shift 없음 또는 정렬 없음)=`ApplySingleSort`(리셋). **표시**: 화살표(▲/▼)는 **컬럼명 앞**, 순번은 **컬럼명 뒤 원문자**(`HeaderCell.OrderText`=①②③…, 정렬된 컬럼마다). 빈 배지는 `Visibility.Collapsed`로 간격 제거. 패널별 독립(HeaderCell) 유지.
