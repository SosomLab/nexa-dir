...
> 🏷️ **릴리스 `0.3.4`** (2026-07-11) — 네비 버튼 상태 정합(세션 복원 직후 동기 PR#18 + hover/pressed/disabled 시각 식별 PR#19). 자산 2종: **포터블 zip + 설치기 setup.exe**(win-x64, self-contained, CI 자동 첨부). 이전: `0.3.3`(BUG-010 수정판)·⚠️`0.3.2`(자산 결함, 사용 금지)·`0.3.1`(zip 최초)·`0.3.0`(M1 후반 소스 태그). [Releases](https://github.com/SosomLab/nexa-dir/releases). (MSIX+서명은 PKG-4 — Store/OV 결정 대기.)
>
...
> **② 도구 모음 그룹화**(PR#10, [docs/44 §5](44-toolbar-and-launcher.md)) — 파일 표시 토글(숨김·도트) 그룹+구분선·순서 설정(▲▼)·플랫 컨텍스트 메뉴(빈영역·탭)·터미널 위치 이동 버튼(도크 한몸)·Copy as name·컨텍스트 메뉴 글꼴 슬롯·탭 전환 도크 미갱신 수정.
> **③ 패널·터미널 UX**(PR#11) — **하단 도크 대원칙**(듀얼=좌↔좌·우↔우/싱글=활성 패널)·분할 위치 저장·복원·"터미널에서 열기(내장)"·터미널 선택 자동 스크롤/우클릭 복사/줄바꿈 옵션(80~1000)·다크 메뉴 바+테마 서브메뉴(**기본 System**).
> **④ 타입어헤드 마감**(PR#12, [docs/32](32-typeahead-find.md) 1~5 완) — 전역 KeyDown 합성(CharacterReceived 비버블 확정)·검색어 HUD(`EphemeralOverlay`, 위치 3×3 피커·입력 옵션 3종)·**터미널 포커스 강탈 근본 수정**·크기 표기(KB/MB/GB+Bytes). 잔여: 한글 IME(§9).
...
> **⑥ 러버밴드 다중 선택**(B-4 부분 — 미선택 행/빈 공간 드래그=밴드·행 히트영역=컬럼 총폭) · **⑦ 탭 UX**(드래그 재정렬·패널 간 이동/Ctrl 복제·삽입 하이라이트·우클릭 메뉴[잠금·고정]·말줄임) · **⑧ breadcrumb 긴 경로 끝 정렬** · 경로·항목 수 헤더 토글 · **⑨ 앱 아이콘**(트리 다중선택+`</>` 컨셉, `scripts/make-app-icon.ps1`).
> **⑩ 테마 시스템 S1**([docs/39](39-theme-system.md)) — 틴트 제거→시맨틱 토큰·라이트 팔레트·구성 메뉴 라이트/다크/시스템 · 상태바=상태 전용(토글은 표시 메뉴) · **파일 목록 가로 스크롤**(넘칠 때만·헤더 동기).
> **⑪ 설정(Preferences) 시스템 PREF-1**([docs/40](40-preferences-system.md)) — `SettingsStore`(`settings.json` 로밍·영속 규율 공용) + `Ctrl+,` 설정 창(모양/레이아웃/메뉴/언어) — **재시작 소실되던 옵션 4벌 영속** · 레이아웃 토글(런처·우 패널)도 `session.json` 영속.
> **⑫ i18n**([docs/42](42-i18n-language-files.md)) — 순수 `Localizer`(폴백 체인) + `LocExtension` 마크업 확장 + 메뉴 바 이관 + 언어 페이지(재시작 전환) · **외부 `.lang` 파일화**(설치 `lang\` + 사용자 `%APPDATA%\NexaDir\lang\`·포맷 심 JSON↔properties·기준 en — 재빌드 없이 언어 추가/수정).
> **⑬ 경로 복사(Copy as path)**([docs/38 §7-5](38-adr-0005-shell-context-menu.md)) — 교차폴더 전체 선택을 클립보드로(셸 단일폴더 한계 우회) + 셸 동사 **제자리 대체**(VerbReplacement) · Alt=POSIX(`/`).
> **문서**: 설정 [40] · 언어팩 [42] · **외부 파일 위치 맵 [43](43-external-files-and-config.md)** · 영속 메커니즘 [34](34-settings-and-session-persistence.md).
>
...
>
> **이전(2026-07-05) · 3차 감사 라운드 `refactor/003-audit` → main 병합.** M1 파일 조작 계층이 크게 채워짐:
> **파일 작업**(복사/이동/완전·휴지통 삭제·이름변경·**새로 만들기** 폴더/파일/바로가기) · **컨텍스트 메뉴**(폴더/파일/빈영역) ·
> **단축키**(Ctrl+C/X/V·Del·F2) · **DnD**(폴더 이동·좌우 패널·탭/폴더 hover 진입·ESC 취소·디스크별 기본·**외부 앱에 파일 열기**(StorageItems)·Alt=경로) ·
> **전송 단일 엔진 `TransferPathsInto`**(DnD·붙여넣기 공용 — **덮어쓰기 확인**[예/모두예/건너뛰기/취소]·**바이트 진행률 + 진행 창**[맨앞·취소·자동닫기 off]) ·
> **OS 클립보드 연동(읽기측)**(탐색기 복사→붙여넣기) · **헤더 정렬**(COL-2/3, 3상태·다중열·좌우 독립) · **확장자/수정 날짜·시간 컬럼**(COL-1/COL-D) ·
> **타입어헤드 찾기**(코어 find_prefix·버퍼·앱 배선) · **자동 갱신 watcher 1차** · **탭 세션 저장/복원**(`session.json`, 요청/수행 분리·Tick 코얼레싱) · **경로 바 환경변수 해석**(%VAR%/$env:).
> 파일 작업은 `Nexa.ViewModels.FileOps`에 중앙화(향후 nexa-ops 이관 seam). **테스트: 코어 green · ViewModels xUnit 57 green · 앱 빌드 0/0.** 실기 QA(Windows) 다수 대기 → [QA-003](QA-003-checklist.md)·[TASKS](TASKS.md). 상세 → [journal 003 worklog](journal/archive/refactor-003-worklog.md).

> 이전(2차): 단계: **M1 진행** — 데이터흐름(F1~5)+레이아웃(F6~9)+**플래그십 초안**(인라인 펼침·컬럼 F10 / 선택 F11·F17) + **패널 탭**(F20·닫기 F22) + **계층 경로 바 `NexaPathBar`**(F23) + 네비(F13·F19·F21) + **숨김/점 파일 토글**(코어 attrs·ABI v2, F24) + **마우스 뒤로/앞으로**(F25) 구현(**F1~F25**, [19](19-implemented-features.md)). 기본 폰트 Segoe UI. **진행 중: C1 코어 트리/선택**(`refactor/001-audit`, ADR [29](29-adr-0004-core-tree-model.md)) — 슬라이스 1 `nexa-tree` 모델(F26) + 슬라이스 2 **C ABI `nexa_tree_*`·ABI v3**(F27). + 슬라이스 3a **호스트 ABI 안전 계층**(로드 시 버전·레이아웃 검사, F28, 감사 A2/A3 정정). + 슬라이스 3b **앱 가상화 재배선**(F18 펼침유지·스크롤 위치 복원) + 슬라이스 **4-1 AC5 10만 노드 코어 벤치**(expand 100k 5.7ms 등 전부 프레임 예산 안 → O(log n) 매핑 불요). + **4-2 탭별 트리 핸들 캐시**(전환 시 재-Open 제거) + **4-3 id→가시 인덱스 조회**(`nexa_tree_index_of`, **ABI v3→v4**) — 클릭 시 O(n) 행 실체화 병목 제거. + **E17 죽은 코드 정리** + **4-4 펼침/접힘 스크롤 보존** + **E19 키보드 캐럿 스크롤 견고화**(오프스크린 추적). **C1 완료 → `refactor/001-audit`를 main에 병합**(`b38e6b3`, 태그 `0.1.0`=머지 전 베이스라인 `6e81734`). **현재: 2차 감사 라운드 `refactor/002-audit`** — 통합 감사(전체 개발범위 5축 점검, [journal](journal/archive/20260702_234558_refactor-002-audit.md)) 후 **트랙 A 성능** 완료: **A-3 [P3] 경로→NodeId 조회**(`nexa_tree_index_of_path`/`expand_path`, **ABI v4→v5**, per-row 마샬 제거) + **A-1 [P1] 열거·펼침 백그라운드화**(`Task.Run` 오프로드+세대 가드 — 대형 폴더 진입 UI 프리즈 제거, NFR-P1/R5) + **A-2 [P2] 펼침/접힘 범위 diff 통지**(전체 Reset·재실체화 제거, 캐시 인덱스 시프트) + **A-4 [P6] 아이콘 LRU 캐시 + 속도 제한 로딩 큐**(`IconKey`+`ShellIconCache` — 화면밖 드롭, 빠른 스크롤 크래시 해소). **트랙 B 구조 리팩토링**: **B-2a `PanelView` 그룹 객체**(`bool left` 이중화 25곳 소거) + **B-1 `Nexa.ViewModels`(net8.0, 플랫폼 무관) 추출**(`PathDisplay`·`NavigationHistory`·`IconKey`) + **C# 테스트 도입**(xUnit **25**, CI `viewmodels` 잡=ubuntu 크로스플랫폼). [BUG-001](BUGS.md) 빈 폴더 이탈 blank는 **✅ 해결**(실험 코드가 원인, 되돌린 뒤 미재현·QA 확인). **테스트: 코어 21(+벤치 1 ignored) · C# 25 green.** MainWindow 955줄(B 리팩토링 지속 대상)
...
**[DEVLOG.md](DEVLOG.md)**(개발 진행 시간 역순 — 통합) · **[MILESTONES.md](MILESTONES.md)**(기능·마일스톤 — 통합) · **[BRANCHES.md](BRANCHES.md)**(브랜치 이력) · **[TODO.md](TODO.md)**(할 일 백로그) · **[BUGS.md](BUGS.md)**(알려진 이슈) · CLAUDE.md(이식 메모리) · 00 비전 · 01 아키텍처 · 02 로드맵 · 03 기능 · 04 트렌드백로그 ·
05 요구사항 · 06 ADR-0001(스택) · 07 플래그십 · 08 경쟁조사 · 09 플러그인 · 10 결정기록 ·
11 개발환경 · 12 포터블 · 13 라이선스 · 14 컨텍스트공유 · 15 개발방법론 · 16 프로젝트구조 ·
17 라이선스인증 · 18 빌드&테스트 · 19 구현기능현황 · 20 UI레이아웃 · 21 ADR-0002(파일뷰) · 22 ADR-0003(뷰/패널모듈) · 23 컬럼시스템 · 24 검색 · 25 일괄리네임 · 26 커맨드팔레트 · 27 경로바컴포넌트 · 28 창위치/세션복원 · 29 ADR-0004(코어트리모델) · 30 용어집(인터롭/배관) · 31 스크롤위치노트 · 34 설정·세션영속 · 38 ADR-0005(셸컨텍스트메뉴) · 39 테마시스템 · 40 설정시스템 · 41 리네임UDF(Python) · 42 i18n언어파일 · 43 외부파일위치맵 · journal/ (작업기록)
