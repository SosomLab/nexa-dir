...
> 갱신: 2026-07-02 (KST) · 단계: **M1 진행** — 데이터흐름(F1~5)+레이아웃(F6~9)+**플래그십 초안**(인라인 펼침·컬럼 F10 / 선택 F11·F17) + **패널 탭**(F20) + 네비(F13·F19·F21) 구현(F1~F21, [19](19-implemented-features.md)). 다음=**코어 VisibleRow 스트림(C1)**·경로바(브레드크럼)·`Nexa.ViewModels` 분리 · 코어 9 tests green
...
- **레이아웃·플래그십 초안 진행** ✅ (F7~F17, 상세 [19](19-implemented-features.md)):
  ...
  | 탭·네비게이션 (F20·F21) | **패널별 탭**(멀티라인·고정크기·`…`·더블클릭 추가·활성 상단 하이라이트, `PanelTab` 탭별 경로/기록/펼침, F20) · **Alt+↑/←/→ 위로·뒤로·앞으로**(F21) |

  단축키는 현재 하드코딩 → **명령 레지스트리+`keybindings.json` 재정의**로 이관 예정([26 §5-4](26-command-palette.md), FR-I2).
...
17 라이선스인증 · 18 빌드&테스트 · 19 구현기능현황 · 20 UI레이아웃 · 21 ADR-0002(파일뷰) · 22 ADR-0003(뷰/패널모듈) · 23 컬럼시스템 · 24 검색 · 25 일괄리네임 · 26 커맨드팔레트 · journal/ (작업기록)
