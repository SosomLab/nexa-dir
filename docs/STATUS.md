...
> 파일 작업은 `Nexa.ViewModels.FileOps`에 중앙화(향후 nexa-ops 이관 seam). **테스트: 코어 green · ViewModels xUnit 57 green · 앱 빌드 0/0.** 실기 QA(Windows) 다수 대기 → [QA-003](QA-003-checklist.md)·[TASKS](TASKS.md). 상세 → [journal 003 worklog](journal/refactor-003-worklog.md).
