...
> 🏷️ **릴리스 `0.3.3`** (2026-07-11) — **0.3.2 자산 결함 수정판**([BUG-010](BUGS.md): 렌더링 깨짐·시작 크래시 → XBF 루트 배치+WinAppSDK 1.8). 자산 2종: **포터블 zip + 설치기 setup.exe**(win-x64, self-contained, CI 자동 첨부). ⚠️ `0.3.2` 자산은 사용 금지(결함 표기). 이전: `0.3.1`(zip 최초)·`0.3.0`(M1 후반 소스 태그). [Releases](https://github.com/SosomLab/nexa-dir/releases). (MSIX+서명은 PKG-4 — Store/OV 결정 대기.)
>
