...
> 상태: **슬라이스 1 구현**(2026-07-08, `feat/toolbar-launcher`) — 버튼·실행·아이콘·시드. CRUD·영속·단축키 배정은 후속.
...
1. **CRUD + 영속**(PREF-6): 설정 창 "런처" 페이지 — 도구 추가/수정/삭제/순서, `settings.json` `Launcher.Items`(라벨·exe·인자·아이콘 오버라이드). 도구 모음도 표시/숨김·순서 사용자화.
