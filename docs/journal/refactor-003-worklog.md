...
- **무엇 · 파일** [MainWindow.xaml](../../app/Nexa.App/MainWindow.xaml):
  ...
  - 편집 TextBox(좌·우 2곳) **`Height="20" Margin="0,-2,0,-2"`**: 20px로 렌더되나 **음수 마진으로 레이아웃 점유는 20−2−2=16px**(아이콘과 동일) → **행이 커지지 않음**(초기 시도 `Margin="0"`은 점유 20px라 행이 26px로 늘어 화면 스트레치 → 음수 마진으로 정정). 렌더 20px가 패딩 안으로 2px씩 넘쳐 행 위·아래 **1px 지점**까지 = 행보다 1px씩 작게.
- **왜 음수 마진**: WinUI에서 음수 마진은 렌더 크기는 유지하되 부모가 잡는 점유 공간을 줄여, "행 높이 유지 + 행보다 작게"를 동시에 달성. 일반 TextBox 조절 난점(MinHeight 32·헤더·×버튼)은 커스텀 `RenameBox` 템플릿으로 이미 제거됨.
- **검증**: WinUI 맥 빌드 불가 → **실기(Windows) 확인** 필요(행 높이 미세 차이 시 `Height`만 조정). 투명 배경이라 편집 중 선택 하이라이트가 테두리 안에 비침(의도).
