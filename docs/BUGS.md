...
## BUG-007 · 내장 터미널 캐럿(커서) 미표시 (☐ BP-T3)

- **심각도**: 중 · **상태**: ☐ 후속(BP-T3 렌더 개선). 관련 [docs/37](37-terminal.md).
...
- **해결 방향**: 렌더 시 커서 위치 셀을 블록/언더라인으로 강조(반전 또는 accent 배경) + 선택적 깜빡임. `VtScreen`에 커서 좌표 노출 API 추가.

## BUG-008 · 내장 터미널 일부 SGR 색 미반영(명령 미리보기 등) (☐ BP-T3)

- **심각도**: 중 · **상태**: ☐ 후속(BP-T2 파서 보강/BP-T3). 관련 [docs/37](37-terminal.md).
...
- **해결 방향**: faint → 전경색 알파/밝기 낮춰 렌더. 미처리 SGR/CSI 보강. (파워라인 글리프는 Nerd Font 적용 — 별도.)
