...
## 9. 재시작 필요 설정 — 확인창 + 자체 재시작 (PREF-9, 2026-07-10)

> 언어처럼 **라이브 적용이 불가능한 설정**(XAML 마크업 확장이 파싱 시점에 문자열을 박제 — docs/42)은
> 변경 직후 확인창을 띄우고, 승인 시 앱이 **스스로 재시작**해 즉시 반영한다. 거부해도 설정은 저장되어
> 다음 시작 때 적용(기존 `pref.lang.restartNote` 동작 그대로).

### 9-1. 진행 과정 (시퀀스)

```text
[설정 창] 언어 라디오 변경
   1) AppSettings.General.Culture = 새 값
   2) Changed() → MainWindow.OnPreferencesChanged()      … 라이브 적용 + MarkSettingsDirty(저장 예약)
   3) MaybePromptRestart(판정 델리게이트)                 … 재시작 필요 설정만 호출(항목별 선언)
        판정 false(원복 등) → 조용히 종료
        판정 true → ContentDialog "재시작 필요"(지금 재시작[기본] / 나중에)
   4) [나중에] → 아무 것도 안 함(저장은 이미 예약됨 → 다음 시작 때 적용)
   5) [지금 재시작] → MainWindow.RestartApp()
        5a) _session.Flush() + _settings.Flush()          … 동기 저장. 必先 — Restart는 Closed/
                                                            ProcessExit 훅 실행을 보장하지 않음
        5b) AppRestart.Restart()
              1차: AppInstance.Restart("")                … WinAppSDK AppLifecycle. 패키지/미패키지 공용.
                                                            성공 시 복귀하지 않음(OS가 프로세스 종료 후 재기동)
              2차(복귀=실패 또는 예외): Process.Start(Environment.ProcessPath) → Application.Exit()
                                                            새 인스턴스 기동까지 실패하면 현 인스턴스 유지
   6) 재기동 → App.OnLaunched → SettingsStore.Load/Apply → Loc.Init(새 Culture)
      → 새 언어로 전 UI 구성 + 세션(탭·패널)은 5a에서 flush된 상태로 복원
```

### 9-2. 판정 규칙 — "정말 달라졌을 때만" 묻는다

- 판정은 **설정값이 아니라 실효값** 비교: `Loc.IsPendingCultureChange`가 설정값(`""`=시스템 포함)을
  `Resolve`로 **실제 언어 코드로 해석**한 뒤 현재 적용 중인 `Localizer.Current.Culture`와 비교한다.
- 따라서 ko→en→ko처럼 **원래 값으로 되돌리면 확인창이 뜨지 않고**, `""`(시스템)↔`"ko"`(시스템=한국어 PC)처럼
  실효 언어가 같은 전환도 묻지 않는다.
- **항목별 선언**: 재시작 필요 설정은 자신의 변경 핸들러에서 `MaybePromptRestart(판정)`을 호출한다
  (판정 로직은 설정마다 다르므로 델리게이트로 위임). 현재 대상=언어 1종, 후속 후보=테마팩 등.

### 9-3. 구성 요소

| 파일 | 역할 |
|---|---|
| `PreferencesWindow.MaybePromptRestart` | 판정 → ContentDialog(지금/나중에) → 승인 시 호스트 재시작 위임 |
| `MainWindow.RestartApp` | **선-flush**(settings.json+session.json 동기 저장) 후 `AppRestart.Restart()` |
| `AppRestart.Restart` | 1차 `AppInstance.Restart`(성공 시 미복귀) → 2차 exe 재기동+`Application.Exit` 폴백 |
| `Loc.IsPendingCultureChange` | 언어 재시작 필요 판정(실효 코드 비교) |
| i18n 키 `restart.*` | 다이얼로그 문자열(title/msg/now/later) — en/ko `.lang` + 임베디드 en |

### 9-4. 설계 근거

- **왜 폴백이 필요한가**: 앱은 현재 미패키지(`WindowsPackageType=None`, 포터블 지향 docs/12).
  `AppInstance.Restart`는 미패키지도 지원하지만(WinAppSDK 1.1+), 런타임 구성에 따라 실패할 수 있어
  실패 사유 무관 **exe 직접 재기동** 폴백을 둔다(단일 인스턴스 강제 없음 → 짧은 공존 무해).
- **왜 선-flush인가**: `AppInstance.Restart`는 프로세스를 즉시 종료하므로 `Closed`/`ProcessExit` 훅의
  flush(§8·PREF-1)가 돈다는 보장이 없다. 방금 바꾼 언어 설정이 유실되면 재시작이 무의미해지므로
  `RestartApp`이 재시작 **전에** 동기 flush를 끝낸다.
