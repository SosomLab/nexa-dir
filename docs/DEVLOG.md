...
- **B-13u Undo/Redo S1 — 탐색기식 Ctrl+Z/Y**: `Nexa.ViewModels.OperationHistory`(스택 2개·redo 무효화·실패=소실+알림) + `MoveBatchOp`/`CopyBatchOp`/`RenameOp`/`CreateOp`(삭제 주입으로 Windows API 격리, **xUnit 10건·총 67 green**). 기록 배선 = `TransferPathsInto` 실수행 쌍(배치=1 트랜잭션)·이름변경·새로만들기 3종. Ctrl+Z/Ctrl+Y(+Ctrl+Shift+Z), 상태바 표기, 완료 후 재로드. 잔여 S2=휴지통 삭제 복원(셸 COM)·nexa-ops 이관. 상세 [journal/2026-07-07.md](journal/2026-07-07.md)·[docs/33 §B-13u](33-file-ops-dnd-design.md).
