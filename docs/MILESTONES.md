...
### 하단 패널 (feat/bottom-panel, BP-1 ✅)
- ✅ 콘텐츠 호스트(정보/미리보기/Hex/터미널 종류 선택·스왑, 정보=현재 폴더) · Ctrl+` 토글 · 상태 세션 저장/복원.
- ☐ 미리보기/Hex 콘텐츠(BP-2) · 터미널 ConPTY(BP-T, 설계 [03 §3-K2b](03-features.md)).
...
### 미착수(M1 잔여)
- ☐ **컨텍스트 메뉴 셸 통합**(`IContextMenu`/`IExplorerCommand`, SHELL) — COM ADR 후보.
- ☐ **퀵 런처 바**(외부 터미널/에디터) · ☐ 하단 임베디드 터미널(설계 동결, BP-T).
