...
| **M1** | ★ 1순위 묶음 — 경로 바·탭/듀얼·플래그십(인라인 트리+교차선택)·컨텍스트 메뉴·퀵 런처 | 🚧 **후반** — 셸 컨텍스트 메뉴 S1 ✅(경로복사 제자리대체 포함, S2/S3 잔여)·Undo/Redo ✅·러버밴드 ✅·테마 ✅·**설정 시스템 PREF-1 ✅**·**i18n(외부 `.lang`) ✅**·**도구 모음+퀵 런처 슬라이스1 ✅**(CRUD/단축키 후속) |
...
### 인프라·설정·영속·i18n (07-08)
- ✅ **설정 시스템 PREF-1**([docs/40](40-preferences-system.md)): `SettingsStore`(`settings.json` 로밍·요청/수행 분리·원자적 쓰기·종료 flush) + `Ctrl+,` 설정 창(모양/레이아웃/메뉴/언어 페이지, 라이브 적용) — **재시작 소실되던 옵션 4벌(테마·표시·메뉴·탭) 영속**. 잔여 PREF-2~7(테마팩·폰트·컬럼·단축키·런처·즐겨찾기).
- ✅ **탭/레이아웃 세션 영속**([docs/34](34-settings-and-session-persistence.md)): `session.json`에 탭·펼침·정렬·잠금/고정·하단 패널·**레이아웃 토글(런처·우 패널)**.
- ✅ **i18n**([docs/42](42-i18n-language-files.md)): 순수 `Localizer`(폴백 체인, xUnit)+`LocExtension` 마크업 확장+메뉴 바 이관+언어 페이지(재시작 전환) · **외부 `.lang` 파일화**(설치 `lang\`+사용자 `%APPDATA%\NexaDir\lang\`·포맷 심 JSON↔properties·기준 en). 잔여: 문자열 전면 이관·번역 언어 추가.
