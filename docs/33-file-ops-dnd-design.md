...
> **구현 상태(2026-07-05)**: **읽기측 부분 구현** — 붙여넣기 활성화(`CanPaste`)와 붙여넣기(`PasteIntoDir`)가
> 앱 내부 클립보드 없으면 **OS 클립보드의 파일(StorageItems)** 을 읽어 처리(탐색기에서 복사→우리 앱 붙여넣기).
> 잘라내기 여부는 "Preferred DropEffect"로 판정. 외부 복사 시 `Clipboard.ContentChanged`로 앱 내부 클립보드를
> 비워 **최신(OS) 우선**. 남은 것: **쓰기측**(우리 복사를 OS 클립보드에 기록 → 탐색기에 붙여넣기) = 아래 옵션.
