...
| `package` | windows-latest | `make-setup.ps1`(setup.exe, Inno=러너 기본 포함) → `dist/*` 아티팩트 + **태그 트리거면 GitHub Release에 자산 자동 첨부**(없으면 릴리스 생성). **태그 push·workflow_dispatch 시만**. 포터블 zip 단계는 ⏸ 잠정 비활성(07-11, [12 §1](12-packaging-portable.md)) |
...
## 4-1. 패키징 — 로컬 산출 (PKG-2/5, [12 §7](12-packaging-portable.md))

```powershell
pwsh scripts/make-setup.ps1                 # → dist/NexaDir-<ver>-setup-win-x64.exe (Inno Setup 필요 — CI 러너 기본 포함)
pwsh scripts/make-portable.ps1              # (⏸ CI 잠정 비활성 — 로컬 산출은 가능) → dist/...-portable-win-x64.zip
```
