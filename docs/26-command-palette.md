...
- **★ 할 일(사용자 요청) — 표시 옵션(설정 화면)**: (a) 그리드 상단 **헤더 정보란**(`현재경로 — N개 항목`, `DirHeader`) **on/off 토글**(`ViewOptions.ShowHeaderInfo`). (b) **상위 이동 시 대상 폴더 정렬 위치** — 가운데/맨 앞/맨 뒤 등(`ViewOptions.UpNavTargetAlign`, 기본 0.5=가운데는 구현됨, 선택 UI만 TODO). (숨김/점 파일 F24와 같은 표시 옵션 그룹.)
