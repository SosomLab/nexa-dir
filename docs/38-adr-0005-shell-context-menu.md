...
### 7-4. Checksum·인코딩 서브메뉴 (구현됨)

- **레지스트리/네이티브 메뉴 서브메뉴 지원**: `CmItemDef.Children` → `CustomItem.Children` → HMENU `MF_POPUP`
  재귀 렌더(리프만 실행, 부모 Visible이 그룹 게이트). 자식 Id는 `parent.child`(예: `checksum.md5`) — 설정 개별 제어.
- **Checksum ▶** (파일 1개↑): **MD5 · SHA-1 · SHA-256 · SHA-384 · SHA-512 · CRC32**(System.IO.Hashing, MIT ·
  표준 빅엔디언 표기). 파일별 "파일명: 해시" 줄(**폴더 제외**), 복사 가능한 대화상자 + "복사" 버튼.
- **인코딩 ▶** (단일 파일): **Base64 인코드 → 클립보드**(16MB 상한). Checksum(단방향 해시)과 범주가 달라
  **별도 메뉴**로 분리(가역 인코딩). 후속: Base64 디코드→파일, URL/Hex 인코딩.
